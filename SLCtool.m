function [adW, MW, W, AC, MC, PAC] = SLCtool(X, T)

% Do Sparce Logistic Classification 
...
iii=0;
W=zeros(size(X,2)+1,K*(K-1)/2)   ; R=[]; ii=0; 
...
%             Ti(f3)=1; Ti(f4)=2;
            ...
            %  Model fitting ----------------------------
            if K~=2
                [W(:,iii), pp, ac(iii)] = HSLR_fit(XG,Ti,[-3.0  1.5],[-4.0  2.3],11);  % Sparse logistic regression
            elseif K==2
                [W(:,iii), pp, ac(iii)] = HSLR_fit(XG,Ti,[-3.0  1.5],[-4  2.0],11);  % Sparse logistic regression
            end
...
%      Z3=[zeros(dimb,1) ;1];
% %      P=Z2\Z3;
%      P=pinv(Z2)*Z3;  % use minimum-norm solution
%      P=P(1:dimb);
%      [I,ii(n)]=max(P);
% end
% 
